...
if($_FILES['file']['name'][0] != ""){
	...
	$sql = 'SELECT * FROM foto_locale WHERE id_locale = "'.$id_locale.'"';
	...
	if(mysqli_num_rows($result) > 0){
	    ...
	    $sql1 = 'DELETE FROM foto_locale WHERE id_locale = "'.$id_locale.'"';
		$connessione->query($sql1);
	}
	
	if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_REQUEST['upload_img_l']) && $_REQUEST['upload_img_l'] == 'Carica immagine'){
		$maxSize = 12000000;
		$acceptType = array(
		'jpeg' => 'image/jpeg',
		'jpg' => 'image/jpeg',
				...
				if ($_FILES['file']['size'][$i] <= $maxSize){ 
					if (in_array($_FILES['file']['type'][$i], $acceptType)){
						...
						//CARICO L'IMMAGINE
							...
							$sql = 'INSERT INTO foto_locale (id_foto_locale, id_locale, id_utente, data_inserimento, nome_galleria, nome_immagine, estensione) VALUES (NULL, "'.$id_locale.'", "'.$_SESSION['user_id'].'", "'.$date.'", "'.$album.'", "'.$nome_completo.'","'.$estensione.'")';
		...
		echo "<script>
			window.location.href = 'locali.php';
		</script>";	
